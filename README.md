...
This is a set of scripts that can be used to run document parsing and text recognition using [dots.mocr](https://huggingface.co/rednote-hilab/dots.mocr) on Princeton's high-performance computing clusters.
...
- Download the dots.mocr model from HuggingFace Hub
- Parse documents and recognize text in the images, saving results as markdown.
...
- type `wget https://raw.githubusercontent.com/tpendragon/vlms-on-hpc/refs/heads/main/setup.sh`
- then `bash setup.sh`
...
To download the dots.mocr model:
```bash
python fetch.py model rednote-hilab/dots.mocr
```
This command saves the location of your downloaded model snapshots in a file called "model_info.json".

I find it helpful to do a test run on the login node to check for errors. With your virtual environment activated, you can run `python main.py`. If everything is set up properly, you'll get an error from vLLM that it can't find the GPU (the login node doesn't have one).
...
You can also download a single IIIF image directly into the `img` folder:

```bash
mkdir -p img
curl -o img/my_image.jpg "<IIIF image URL>"
```

All images will be saved in the img folder. Markdown output files will be saved in the markdown folder.
...
The main things to note and adjust are:
- your job name
...
When you have the model downloaded and PDFs ready:
```bash
...
To move your images and text off the Adroit servers, you can do the following
...
`/scratch/network/myusername/vlms-on-hpc/markdown`

Open the terminal on your computer.
You can copy the folder from Adroit to your computer with the `scp` command.
`scp <origin> <destination>`
for example,
`scp myusername@adroit.princeton.edu:/scratch/network/myusername/vlms-on-hpc/markdown/ ~/Downloads/`
